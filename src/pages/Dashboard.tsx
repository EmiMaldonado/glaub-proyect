...
import { MessageCircle, Brain, TrendingUp, Users, Calendar, Plus, History, Settings, Target, Lightbulb, Share2, UserCheck } from "lucide-react";
...
const Dashboard = () => {
  ...
  const loadDashboardData = async () => {
    ...
    try {
      ...
      const {
        data: insights
      } = await supabase.from('key_insights').select(`
          *,
        ...
        `).eq('conversation.user_id', user.id).order('created_at', {
        ascending: false
      });
          ...
          {lastConversation ? <div className="space-y-3">
              <div className="grid grid-cols-3 gap-4 text-sm">
                <div>
                  ...
                  <p className="font-semibold">{lastConversation.duration_minutes || 15} min</p>
                </div>
                <div>
