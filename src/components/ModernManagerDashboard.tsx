...
import { Users, TrendingUp, MessageCircle, Target, Settings, BarChart3, User } from 'lucide-react';
...
const ModernManagerDashboard: React.FC = () => {
  ...
  } = useTeamAnalytics(managerProfile?.id || '', teamMembers);
  useEffect(() => {
    if (user) {
      loadManagerData();
    }
  ...
  const loadManagerData = async () => {
    ...
    try {
      ...
      const {
        data: members,
        error: membersError
      } = await supabase.from('profiles').select('*').eq('manager_id', profile.id).order('created_at', {
        ascending: false
      });
  ...
  return <div className="container mx-auto px-4 py-8 space-y-8">
      ...
      {/* Welcome Messages */}
      <WelcomeMessages userRole="manager" teamName={managerProfile.team_name} isNewManager={teamMembers.length === 0} hasTeamMembers={teamMembers.length > 0} onStartInviting={() => setActiveTab('management')} onConfigureSettings={() => setActiveTab('management')} />
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <Card>
              ...
              <CardContent className="space-y-4">
                {teamMembers.length === 0 ? <div className="text-center py-8">
                    ...
                    {teamMembers.map(member => <div key={member.id} className="flex items-center justify-between p-3 border rounded-lg cursor-pointer hover:bg-muted/50" onClick={() => {
            ...
            <Card>
              ...
              <CardContent className="p-0">
                {analyticsLoading ? <div className="text-center py-8 px-6">
                    <BarChart3 className="mx-auto h-12 w-12 text-muted-foreground/50" />
                    <p className="text-muted-foreground mt-4">Loading team analytics...</p>
                  </div> : analyticsData && teamMembers.length > 0 ? <div className="p-6">
                    <TeamAnalyticsDashboard analyticsData={analyticsData} dateRange={{
        ...
        <TabsContent value="insights" className="space-y-6">
          <ManagerInsightsDashboard teamMembers={teamMembers} managerId={managerProfile.id} selectedMember={selectedMember} onMemberSelect={setSelectedMember} />
        </TabsContent>

        {/* Recommendations Tab */}
        <TabsContent value="recommendations" className="space-y-6">
          <div className="grid grid-cols-1 gap-6">
            <TeamRecommendationsDashboard managerId={managerProfile.id} teamMembers={teamMembers} />
            
            {selectedMember && <IndividualRecommendationsDashboard managerId={managerProfile.id} member={selectedMember} />}
          </div>
        </TabsContent>
        ...
        <TabsContent value="management" className="space-y-6">
          <div className="space-y-6">
            ...
            <AutoConfigSummary userRole="manager" onConfigureManually={() => setActiveTab('management')} />
            
            {/* Notifications for Manager */}
            <NotificationSystem maxDisplayed={3} />
            
            {/* Team Management Interface */}
            <TeamManagementInterface managerProfile={managerProfile} teamMembers={teamMembers} onTeamUpdate={handleTeamUpdate} />
