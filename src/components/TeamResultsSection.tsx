...
import { CheckCircle2, Lightbulb, Users, User, RefreshCw, TrendingUp, AlertCircle, ChevronDown } from 'lucide-react';
      ...
      <CardContent>
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          <div className="flex gap-8 border-b border-border mb-6">
            <button onClick={() => setActiveTab('overview')} className={`pb-2 text-base font-medium border-b-2 transition-colors ${activeTab === 'overview' ? 'border-[#BCD2C8] text-[#BCD2C8]' : 'border-transparent text-muted-foreground hover:text-foreground'}`}>
              Team overview
            </button>
            <button onClick={() => setActiveTab('individual')} className={`pb-2 text-base font-medium border-b-2 transition-colors ${activeTab === 'individual' ? 'border-[#BCD2C8] text-[#BCD2C8]' : 'border-transparent text-muted-foreground hover:text-foreground'}`}>
          ...
          <TabsContent value="overview" className="space-y-6 mt-6">
            ...
            <div className="space-y-4">
              ...
              {loading ? <div className="space-y-3">
                  ...
                  {teamStrengths.map((strength, index) => <li key={index} className="flex items-start gap-2">
          ...
          <TabsContent value="individual" className="space-y-6 mt-6">
              ...
              if (member) onMemberSelect(member);
                ...
                <SelectTrigger className="w-full bg-background border border-input z-50">
                  <SelectValue placeholder="Choose a team member">
                    {selectedMember && <div className="flex items-center gap-3">
                ...
                <SelectContent className="bg-background border border-input shadow-lg z-50">
                  {teamMembers.map(member => <SelectItem key={member.id} value={member.id} className="cursor-pointer hover:bg-muted focus:bg-muted">
            ...
            {selectedMember && <div className="pt-4 border-t">
                <h4 className="font-medium mb-3">
                  Individual Analysis: {selectedMember.display_name || selectedMember.full_name}
                </h4>
                
                {analyticsData ? <div className="space-y-4">
                    {/* Member Activity Stats */}
                    <div className="grid grid-cols-2 gap-4">
                      <div className="p-3 bg-muted/50 rounded-lg text-center">
                        <div className="text-lg font-semibold text-primary">
                          {analyticsData.distribution.sessionsByMember.find(m => m.name === (selectedMember.display_name || selectedMember.full_name))?.sessions || 0}
                        </div>
                    ...
                    <div className="p-4 border rounded-lg">
                      ...
                      <div className="text-sm text-muted-foreground">
                        {(() => {
                    const memberSessions = analyticsData.distribution.sessionsByMember.find(m => m.name === (selectedMember.display_name || selectedMember.full_name))?.sessions || 0;
                    if (memberSessions === 0) {
        ...
        <div className="mt-8 pt-6 border-t space-y-4">
          ...
          {loading ? <div className="space-y-4">
              {[1, 2, 3].map(i => <div key={i} className="animate-pulse p-4 border rounded-lg">
                  <div className="h-4 bg-muted rounded mb-2"></div>
                  <div className="h-3 bg-muted rounded w-3/4"></div>
                </div>)}
            </div> : <div className="space-y-4">
              {recommendations.map((rec, index) => <div key={index} className="p-4 border rounded-lg space-y-3">
