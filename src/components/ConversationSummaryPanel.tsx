        ...
        <div className={`${isCompact ? 'p-3 space-y-3' : 'p-4 space-y-6'}`}>
          ...
          {conversationContext?.keyInsights?.length > 0 && <Card className="p-4">
              ...
              <div className="space-y-2">
                {conversationContext.keyInsights.map((insight: string, index: number) => <div key={index} className="text-xs p-2 rounded-md bg-primary/5 border border-primary/10">
          ...
          {isSessionCompleted && categorizedSummary && <>
              ...
              {categorizedSummary.insights.length > 0 && <Card className="p-4 border-blue-200 bg-blue-50/30">
                  ...
                  <div className="space-y-1">
                    {categorizedSummary.insights.map((item, index) => <p key={index} className="text-xs text-blue-700 leading-relaxed">
                        {item}
                      </p>)}
                  </div>
                </Card>}
              ...
              {/* Summary Category */}
              {categorizedSummary.summary.length > 0 && <Card className="p-4 border-green-200 bg-green-50/30">
                  <div className="flex items-center gap-2 mb-3">
                    <FileText className="h-4 w-4 text-green-600" />
                    <h4 className="text-sm font-medium text-green-800">Resumen de Conversación</h4>
                  </div>
                  <div className="space-y-1">
                    {categorizedSummary.summary.map((item, index) => <p key={index} className="text-xs text-green-700 leading-relaxed">
                        {item}
                      </p>)}
                  </div>
                </Card>}
              ...
              {/* Strengths Category */}
              {categorizedSummary.strengths.length > 0 && <Card className="p-4 border-purple-200 bg-purple-50/30">
                  <div className="flex items-center gap-2 mb-3">
                    <CheckCircle className="h-4 w-4 text-purple-600" />
                    <h4 className="text-sm font-medium text-purple-800">Puntos Fuertes</h4>
                  </div>
                  <div className="space-y-1">
                    {categorizedSummary.strengths.map((item, index) => <p key={index} className="text-xs text-purple-700 leading-relaxed">
                        {item}
                      </p>)}
                  </div>
                </Card>}
              ...
              {/* Follow-up Category */}
              {categorizedSummary.followUp.length > 0 && <Card className="p-4 border-orange-200 bg-orange-50/30">
                  <div className="flex items-center gap-2 mb-3">
                    <Target className="h-4 w-4 text-orange-600" />
                    <h4 className="text-sm font-medium text-orange-800">Plan de Seguimiento</h4>
                  </div>
                  <div className="space-y-1">
                    {categorizedSummary.followUp.map((item, index) => <p key={index} className="text-xs text-orange-700 leading-relaxed">
                        {item}
                      </p>)}
                  </div>
                </Card>}
          ...
          {oceanSignals && <Card className="p-4">
              <div className="flex items-center gap-2 mb-3">
                <Heart className="h-4 w-4 text-primary" />
                <h4 className="text-sm font-medium">Análisis de Personalidad</h4>
              </div>
              <div className="space-y-3">
                {Object.entries(oceanSignals).map(([trait, value]: [string, any]) => <div key={trait} className="space-y-1">
                    <div className="flex justify-between text-xs">
                      <span className="text-muted-foreground capitalize">
                        {trait === 'openness' ? 'Apertura' : trait === 'conscientiousness' ? 'Responsabilidad' : trait === 'extraversion' ? 'Extraversión' : trait === 'agreeableness' ? 'Amabilidad' : 'Neuroticismo'}
                      </span>
