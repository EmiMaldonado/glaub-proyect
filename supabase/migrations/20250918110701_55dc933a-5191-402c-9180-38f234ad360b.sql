...
-- Paso 1.6: Actualizar tabla invitations para mejorar el sistema
-- Agregar columnas necesarias si no existen
DO $$
BEGIN
  -- Verificar y agregar invitation_type como enum
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'invitation_type_enum') THEN
    CREATE TYPE invitation_type_enum AS ENUM ('team_member', 'manager_request');
  END IF;
  
  -- Actualizar invitation_type para usar enum si es text
  ALTER TABLE invitations ALTER COLUMN invitation_type TYPE invitation_type_enum 
